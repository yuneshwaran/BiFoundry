...
def _as_dict(value):
    """Convert value to dict, handling Pydantic models and dicts."""
...
def _safe_name(value, fallback="Draft"):
    """Create a filesystem-safe name by removing/replacing illegal characters."""
...
def _read_one(conn, stmt):
    """Execute statement and return first row as dict, or None."""
...
def _load_semantic_model_row(conn, semantic_model_row_id):
    """Load semantic model row from either PowerBI or local models table."""
...
def _semantic_model_package_from_files(raw_dataset=None, file_rows=None):
    """Reconstruct PBIP package from stored file components."""

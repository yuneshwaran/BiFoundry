...
function resolveTemplateFromMap(templateMap, templateKey) {
  if (templateKey === null || templateKey === undefined) return null;
  return (
...
export default function CanvasEditorPage() {
  ...
  // Track the server data identity to prevent stale re-hydration
  const lastServerDataRef = useRef(null);
  // Track whether we've done the initial hydration for this project
  const hasHydratedRef = useRef(false);
  // Save debounce refs
  const saveTimerRef = useRef(null);
  const saveInFlightRef = useRef(false);
  const lastSavedSnapshotRef = useRef('');
  // Guard: block re-hydration while a save is in flight or recently completed
  ...
  const projectQuery = useQuery({
    ...
    // Do not auto-refetch on window focus — that's what causes the revert
  ...
  // --- HYDRATION: only hydrate on initial load or explicit server refresh ---
  useEffect(() => {
    if (!projectQuery.data) return;

    // Same server data object reference — nothing changed server-side, skip
    if (lastServerDataRef.current === projectQuery.data) return;

    // Already hydrated and the canvas is dirty (user has unsaved changes) — skip
    // unless this is brand new data from an explicit refetch after a mutation
    if (hasHydratedRef.current && isDirty) return;

    // Suppress if a save just completed (brief window where isDirty=false but
    // store already has the right data)
    if (suppressHydrationRef.current) return;

    lastServerDataRef.current = projectQuery.data;
    hydrateReport(projectQuery.data);
    hasHydratedRef.current = true;

    // Seed the saved-snapshot baseline so the autosave diff starts clean
    lastSavedSnapshotRef.current = JSON.stringify(
      buildCanvasSaveSnapshot(
  ...
  useEffect(() => {
    if (fieldsQuery.data) {
      ...
      if (nextFields.length) setFields(nextFields);
    }
  ...
  const selectedVisual = useMemo(() => {
    if (!activePage || !selectedVisualId) return null;
  ...
  // --- AUTOSAVE: debounced diff-based save ---
  useEffect(() => {
    if (!isDirty || !Number.isFinite(numericProjectId)) return undefined;
    ...
    saveTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (
          !lastSnapshot ||
          lastSnapshot.reportName !== nextSnapshot.reportName ||
          JSON.stringify(lastSnapshot.settings) !== JSON.stringify(nextSnapshot.settings)
        ) {
        ...
        // Release the suppression after a short delay so any in-flight
        // React Query background fetches don't clobber the store
        setTimeout(() => {
          suppressHydrationRef.current = false;
        }, 2000);
      }
  ...
  const createPageMutation = useMutation({
    ...
    onSuccess: async (created) => {
      // Extract the newly created page from the response
      const createdPages = created?.pages || [];
      const newestPage =
        createdPages.find((page) => !pages.some((existing) => existing.id === page.id)) ||
        createdPages[createdPages.length - 1] ||
        null;

      if (newestPage) {
        // Add only the new page to local state; don't replace everything
        addPage({
          ...newestPage,
          visuals: newestPage.visuals || [],
        });
        setActivePageId(newestPage.id);

        // Update the saved snapshot baseline to include the new page so the
        // autosave diff doesn't immediately try to "save" the newly added page
        // (it was already persisted by the createProjectPage call above)
        const updatedPages = [
          ...pages,
          {
            ...newestPage,
        ...
        lastSavedSnapshotRef.current = JSON.stringify(
          buildCanvasSaveSnapshot(reportName, settings, updatedPages),
        );
      }

      setNotice(`Created ${newestPage?.display_name || newestPage?.name || 'page'}.`);
    },
  ...
  const createVisualMutation = useMutation({
    ...
    onSuccess: async (created, variables) => {
      // Find the newly created visual from the response pages
      const createdPages = created?.pages || [];
      const responsePage = createdPages.find((page) => page.id === variables.pageId);
      const currentPageVisualIds = new Set(
        (pages.find((p) => p.id === variables.pageId)?.visuals || []).map((v) => v.id),
      );
      const newVisual = (responsePage?.visuals || []).find((v) => !currentPageVisualIds.has(v.id));
      ...
      if (newVisual) {
        ...
        addVisualLocal(variables.pageId, {
          ...newVisual,
          template_key: newVisual.template_key || template?.template_key || String(variables.templateRef),
          name: newVisual.visual_name || newVisual.name,
          visual_name: newVisual.visual_name || newVisual.name,
          x: Number(newVisual.x ?? newVisual.grid_position?.col ?? 0),
          y: Number(newVisual.y ?? newVisual.grid_position?.row ?? 0),
          w: Number(newVisual.w ?? newVisual.grid_position?.w ?? template?.default_width ?? 3),
          h: Number(newVisual.h ?? newVisual.grid_position?.h ?? template?.default_height ?? 2),
          bindings: newVisual.bindings || newVisual.field_bindings || {},
          config: newVisual.config || newVisual.format_config || {},
        });

        // Update saved snapshot baseline for the new visual
        const currentPage = pages.find((p) => p.id === variables.pageId);
  ...
  const deletePageMutation = useMutation({
    ...
    onSuccess: async (_result, pageId) => {
      ...
      // Update baseline to exclude deleted page
  ...
  const deleteVisualMutation = useMutation({
    ...
    onSuccess: async (_result, variables) => {
      ...
      // Update baseline to exclude deleted visual
      const updatedPages = pages.map((page) =>
        page.id === variables.pageId
          ? { ...page, visuals: (page.visuals || []).filter((v) => v.id !== variables.visualId) }
          : page,
      );
      lastSavedSnapshotRef.current = JSON.stringify(
        buildCanvasSaveSnapshot(reportName, settings, updatedPages),
  ...
  const handleMovePage = (pageId, direction) => {
    ...
    if (index < 0 || targetIndex < 0 || targetIndex >= nextPages.length) return;
  ...
  const handleLayoutChange = (layout) => {
    ...
    layout.forEach((item) => {
      ...
      updateVisualLocal(activePage.id, visualId, { x: item.x, y: item.y, w: item.w, h: item.h });
    });
  };
  ...
  const handleDeleteVisual = (visualId) => {
    if (!activePage || !window.confirm('Delete this visual?')) return;
  ...
  const handleAssignField = (field) => {
    ...
    const candidateSlots = orderedSlots(selectedTemplate).filter((slot) =>
      slotSupportsField(slot, normalizedField),
    );

    if (!candidateSlots.length) {
      setError(
        `No available slot on ${selectedTemplate.name || selectedTemplate.template_key} can accept ${normalizedField.label}.`,
      );
      return;
    }
    ...
    const targetSlot =
      candidateSlots.find(
        (slot) =>
          !currentBindings[slot.role] ||
          (slot.multi &&
            (!Array.isArray(currentBindings[slot.role]) ||
              currentBindings[slot.role].length === 0)),
      ) || candidateSlots[0];
    ...
    <div className="app-shell">
      <header className="hero">
        <div className="hero__content">
          ...
          <p>
            Drag a starter visual onto the canvas, or pick a template from the gallery and bind
            semantic fields from the browser. The project stays saved in the PBIP-ready `projects`
            flow.
          </p>
        ...
        <div className="hero__status">
          ...
          <div className="status-chip status-chip--muted">
            {activePage ? `${activePage.visuals?.length || 0} visuals` : 'No page selected'}
          </div>
      ...
      <main className="workspace page-grid page-grid--builder">
          ...
          <VisualPalette
            ...
            onDragStart={(event, template) => {
              ...
              event.dataTransfer.setData(
                'application/x-bifoundry-template',
        ...
        <div className="page-column page-column--right">
          <PropertiesPanel
            report={{ id: numericProjectId, name: reportName, settings, pages }}
